...
// -- When first called after starting, get a green screen for about 1 second. Everythig fine after that
    ...
    if (this.camera.streaming_enabled == true && this.camera.online == true) {
        ...
        if (this.ffmpegAudioReturn.stdout != null) {
            this.ffmpegAudioReturn.stdout.on("data", (chunk) => {
               ...
               // this.__AudioPayload(Buffer.concat([chunk, Buffer.from([])]));
        ...
        this.camera.capabilities.forEach((element) => {
            if (element.startsWith("streaming.cameraprofile")) {
                ...
                if (index == -1 && this.quality != StreamProfile[profile]) {
                    // Profile isnt the primary profile, and isnt in the list, so add to list of others
...
NexusStreamer.prototype.__handlePlaybackEnd = function(payload) {
    ...
    }, {session_id: 0, reason: 1});

    // Nexusstreamer Playback ended with code '%s'", packet.reason);
    if (packet.reason === Reason.ERROR_TIME_NOT_AVAILABLE && this.liveStreaming == false) {
        setTimeout(() => {
            this.startPlayback();
        }, 1000);
    }
...
NexusStreamer.prototype.__handleNexusData = function(data) {
    // Process the rawdata from our socket connection and converts into approprite nexus packets to take action against
    ...
    var headerLength = 3;
    let length = this.pendingBuffer.readUInt16BE(1);
    ...
    if (this.pendingBuffer.length >= payloadEndPosition) {
        ...
        payload.destroy();  // no longer need proto buffer, so remove it
